...
module.exports = function buildJSDev(gulp, plugins) {
    return function () {
        ...
        function copyCssFiles() {
            return gulp.src(path_1.join(config_1.APP_SRC + '/assets/sass/', '**', '*.css'))
                .pipe(gulp.dest('dist/prod/assets/sass/'));
        }
        function copyFontsFiles() {
            return gulp.src(path_1.join(config_1.APP_SRC + '/assets/components/ProgressButtonStyles/fonts/icomoon', '**', '*.*'))
                .pipe(gulp.dest('dist/prod/fonts/'));
        }
        function copyCkeditorFiles() {
            return gulp.src(path_1.join('./node_modules/ckeditor/', '**', '*.*'))
