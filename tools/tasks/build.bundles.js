...
module.exports = function bundles(gulp, plugins) {
    return function () {
        return merge(bundleShims(), bundleApp(), bundleRef());
        function getShims() {
        ...
        function getref() {
            var libs = config_1.PROD_DEPENDENCIES
                .filter(function (d) { return /\.js$/.test(d.src); });
            return libs.filter(function (l) { return l.inject === 'ref'; })
                .map(function (l) { return l.src; });
        }
        function bundleRef() {
            return gulp.src(getref())
                .pipe(plugins.uglify({
                mangle: false
            }))
                .pipe(plugins.concat(config_1.JS_PROD_REF_BUNDLE))
                .pipe(gulp.dest(config_1.JS_DEST));
        }
        function bundleApp() {
