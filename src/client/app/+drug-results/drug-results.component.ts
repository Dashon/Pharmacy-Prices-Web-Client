...
export class DrugResultsComponent {
    ...
    getDrugs(text) {
        this.formChildren = [];
        return this.callApi('http://localhost:3000/api/v1/drugs?name_prefix=' + text).map(res => res.json())
            .map((el)=> {
                var uniqueItems = {};
                this.formChildren = [];
    ...
    getUsers() {
        this.callApi('http://localhost:3000/api/v1/pharmacies?distance_from_zip=60640');
    }
